...
class YanzhengmaFrame(wx.Frame):
    def __init__(self):
        ...
        wx.Frame.__init__(self, None, 18,'Price',size=Yanzhengmasize, pos=Pos_yanzhengmaframe ,style=wx.FRAME_TOOL_WINDOW|wx.STAY_ON_TOP
                          )
...
#1秒之后查找是否有确定，找不到就关闭放大器
        ...
        if yanzhengma_view :
            ...
            yan_change=yanzhengma_hash.get(yan_hash,0)
            if  yanzhengma_count>=10 and not yan_change:   #1s之后，如果字典里没有就更新库  有的话说明没有变化，不动作
                yanzhengma_hash[yan_hash]=1
            if not yan_change:
                try:
