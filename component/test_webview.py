...
class TestPanel(wx.Panel):
    def __init__(self, parent):
        ...
        self.current = "www.foxavideo.com"
    ...
    def OnWebViewNavigating(self, evt):
        ...
        if evt.GetURL() == 'http://www.microsoft.com/':
            if wx.MessageBox("Are you sure you want to visit Microsoft?", style=wx.YES_NO | wx.ICON_QUESTION) == wx.NO:
    ...
    def OnWebViewError(self, evt):
        self.frame.SetStatusText("error happened here")
    ...
    def OnWebViewLoaded(self, evt):
        ...
        if (self.wv.GetCurrentURL() == 'about:blank'):
            return
        else:
            # wx.MessageBox( self.wv.GetCurrentURL(),  'info', wx.OK | wx.ICON_INFORMATION )
            # self.wv.RunScript("alert('Hello World!');");
            self.wv.RunScript(
                "var ele=document.getElementsByName('apptDetails.apptType'); if ( ele == None ) { alert( 'get nothing' ); } else { alert( 'got something' ); }")
...
def main():
    app = wx.App()
    frm = wx.Frame(None, title="html2.WebView sample", size=(1024, 768))
    frm.CreateStatusBar()
    pnl = TestPanel(frm)
    frm.Show()
    app.MainLoop()
