...
url1="http://127.0.0.1:8020/redo/%E4%B8%BB%E7%95%8C%E9%9D%A2.html?__hbt=1500278201460"
...
for i in range(len(Position)):
    Position[i][0] = Px1 + P_relative[i][0]+ix
    Position[i][1] = Py2 + P_relative[i][1]+iy
#转换为基于浏览器左上角坐标相对位置
#按钮微调幅度--为国拍调整位置做准备
...
py_price=260+iy
#price框放置位置
px_priceframe=220-191+ix
py_priceframe=510+iy
#time放置位置
px_timeframe=400-35+ix
py_timeframe=460+iy  #460
#验证码位置
px_yanzhengmaframe=400-215+ix
py_yanzhengmaframe=460+iy
#出价截取大小
px_mini=200
py_mini=40
#价格框大小
...
Timesize=[200,50]
#最低成交价
px_lowestprice=206+ix  #截图相对位置
py_lowestprice=412+iy
#计算确认键位置
px_confirm=656+ix
py_confirm=475+iy
#计算刷新位置
...
def keymodify(num):
    win32api.keybd_event(key_num[num], 0, 0, 0)
    win32api.keybd_event(key_num[num], 0, win32con.KEYEVENTF_KEYUP, 0)
...
#动态显示价格
    def Price_view(self,event):
        ...
        #     price_view=False
        #     price_on=True
        global tijiao_num,tijiao_one
        if moni_second<one_time1 :
            strategy_on=True
            chujia_on=True
            tijiao_on=False
            tijiao_num = 1  # 初始化
...
############################最低价显示开关
            ...
            try:
                ...
                self.lowestframe.Show(False)  #最低价显示
            except:
                pass
