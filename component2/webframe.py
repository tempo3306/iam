...
from component.variable import init_pos,  get_dick
import logging
logger = logging.getLogger()

class ButtonPanel(wx.Panel):
    def __init__(self, parent, webstatus_label, moni):
        size = V_global.buttonpanel_size
        pos = V_global.buttonpanel_pos
        ...
        if not moni:
            guopai_dianxin = V_global.guopai_dianxin
            if guopai_dianxin:
                urlchange_dianxin_label = V_global.urlchange_dianxin_label
                ...
                urlchange_nodianxin_label = V_global.urlchange_nodianxin_label
...
class HtmlPanel(wx.Panel):
    def __init__(self, parent, moni):
        ...
        htmlpanel_pos = V_global.htmlpanel_pos
...
class BottomeStatusbarPanel(wx.Panel):
    def __init__(self, parent, moni):
        bottomestatusbarpanel_size = V_global.bottomestatusbarpanel_size
        bottomestatusbarpanel_pos = V_global.bottomestatusbarpanel_pos
    ...
    def Modify(self):  # 更新
        ...
        dc.Clear()
        register_label = V_global.register_label
...
class CurrentStatusFrame(wx.Frame):
    def __init__(self, parent):
        ...
        CurrentStatusFrameSize = V_global.CurrentStatusFrameSize
...
class CurrentStatusPanel(wx.Panel):
    ...
    def Modify(self):  # 更新
        ...
        ##当前时间label
        currenttime_label = V_global.currenttime_label
        dc = wx.BufferedDC(wx.ClientDC(self))  # ClientDC客户区  ，BufferedDC双缓冲绘图设备
        a_time = V_global.a_time
        ...
        yanzhengma_view = V_global.yanzhengma_view
        final_stage = V_global.final_stage ##判断是不是11点之后
        ...
        if findpos_on or yanzhengma_view or not final_stage: ##final_stage 是否处于修改出价阶段
            ...
            lowestpricelabel = V_global.lowestpricelabel
            ...
            if userprice and tijiao_on:  ##提交状态
                ...
                if smartprice_chujia:
                    ...
                    currenttime = V_global.a_time
                    if tijiao_num == 1:
                        one_time1 = V_global.one_real_time1
...
class WebFrame(wx.Frame):
    def __init__(self, px, py, id, name, tablabel, moni):  # name:窗口显示名称
        ...
        webstatus_label = V_global.moni_webstatus_label
    ...
    def onekey_login(self):
        bidnumber_js = V_global.bidnumber_js
        bidpassword_js = V_global.bidpassword_js
        idcard_js = V_global.idcard_js
    ...
    def Yanzhengma_scale(self):
        ...
        if yanzhengma_scale:
            yanzhengma_close = V_global.yanzhengma_close
            if yanzhengma_close:
                try:
            ...
            yanzhengma_view = V_global.yanzhengma_view
            imgpos_yanzhengma = V_global.imgpos_yanzhengma
            Yanzhengmasize = V_global.Yanzhengmasize
            #验证码放大是否需要刷新
            if yanzhengma_view:
                V_global.yanzhengma_close = False
                path = V_global.path
                yanpath = path + "\\yanzhengma.png"
                cut_pic(imgpos_yanzhengma, Yanzhengmasize, yanpath)  # 直接调用得到 png 保存图片
                try:
                    yanpath = V_global.yanpath
    ...
    def OnClose(self, event):
        V_global.web_on = False
        V_global.view_time = False
        V_global.moni_on = False
        V_global.guopai_on = False
        self.yanzhengmaframe.Show(False)
        self.currentstatusframe.Show(False)
        event.Skip()
        id = V_global.topframe
