...
from component.variable import  V_global, remote_variables, get_id_hash
...
class AccountPanel(wx.Panel):
    ...
    def OnLogin(self, event):
        ...
        if username == "":
            ...
            V_global.Username = username ## 保存用户输入的账号密码
            V_global.Password = password
...
class Identify_codePanel(wx.Panel):
    ...
    def OnLogin(self, event):
        ...
        V_global.diskid = get_id_hash(diskid) ##sha1 hash化
        Identify_code = self.code_userText.GetValue()
        if Identify_code == "":
            wx.MessageBox('请输入激活码！')
            self.code_userText.SetFocus()
        else:
            V_global.Identify_code = Identify_code ## 保存用户输入的账号密码
...
class LoginFrame(wx.Frame):
    ...
    def connect_success(self):
        ...
        if login_result['result'] == 'login success':
            ...
            V_global.ip_address = ip_address ##设置IP
            ...
            V_global.start_time = start_time
            ...
            if Identify_code == '123456':  ##这里作为测试用
                ...
                V_global.url_dianxin = login_result['url_dianxin']
                V_global.url_nodianxin = login_result['url_nodianxin']
