...
class MyApp(wx.App):
    ...
    def __init__(self, redirect=True, filename=None, useBestVisual=False, clearSigInt=True, processes=[], taskqueue=[],
                 donequeue=[], tasks=[]):
        """
        Initialise the App.
        """
...
if __name__ == '__main__':
    ...
    app = MyApp(redirect=True, filename='wxsimpler_mp.stderr.log', processes=Processes, taskqueue=taskQueue,
                donequeue=doneQueue, tasks=Tasks)
    app.MainLoop()
...
# def myWorker(a, b):
#     time.sleep(10)
#     wx.CallAfter(pub.postMessage, "chufa")
